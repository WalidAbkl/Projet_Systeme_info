...
def insert_Electricite(date,prix,quart_d_heure):
	conn = sqlite3.connect("projet_info.db")
	cur = conn.cursor()
	sqlQuery="INSERT OR IGNORE INTO Electricite (date,prix,quart_d_heure) "
	sqlQuery+=f"VALUES ('{date}',{prix},{quart_d_heure})"
	cur.execute(sqlQuery)
	conn.commit()
	conn.close()
...
def insert_Operateur(id_operateur,nom,mail):
	conn = sqlite3.connect("projet_info.db")
	cur = conn.cursor()
	sqlQuery="INSERT OR IGNORE INTO Operateur (id_operateur,nom,mail) "
	sqlQuery+=f"VALUES ({id_operateur},'{nom}','{mail}')"
	cur.execute(sqlQuery)
	conn.commit()
	conn.close()

# INSERT INTO Machine
def insert_Machine(duree_cycle,puissance,id_operateur):
	conn = sqlite3.connect("projet_info.db")
	cur = conn.cursor()
	sqlQuery="INSERT OR IGNORE INTO Machine (duree_cycle,puissance,id_operateur) "
	sqlQuery+=f"VALUES ({duree_cycle},{puissance},{id_operateur})"
	cur.execute(sqlQuery)
	conn.commit()
	conn.close()
...
def select_Electricite(WHERE):
	conn = sqlite3.connect("projet_info.db")
	cur = conn.cursor()
	sqlQuery="SELECT id_electricite,date,prix,quart_d_heure FROM Electricite"
	if WHERE.strip()!="":
		sqlQuery+=f" WHERE {WHERE}"
	cur.execute(sqlQuery)
	rows = cur.fetchall()
...
def select_Machine(WHERE):
	conn = sqlite3.connect("projet_info.db")
	cur = conn.cursor()
	sqlQuery="SELECT id_machine,duree_cycle,puissance,id_operateur FROM Machine"
	if WHERE.strip()!="":
		sqlQuery+=f" WHERE {WHERE}"
	cur.execute(sqlQuery)
	rows = cur.fetchall()
...
def update_Electricite(id_electricite,date,prix,quart_d_heure,WHERE):
	conn = sqlite3.connect("projet_info.db")
	cur = conn.cursor()
	sqlQuery=f"UPDATE Electricite SET id_electricite = {id_electricite},date='{date}',prix = {prix},quart_d_heure = {quart_d_heure}"
	if WHERE.strip()!="":
		sqlQuery+=f" WHERE {WHERE}"
	cur.execute(sqlQuery)
	conn.commit()
...
def update_Machine(id_machine,duree_cycle,puissance,id_operateur,WHERE):
	conn = sqlite3.connect("projet_info.db")
	cur = conn.cursor()
	sqlQuery=f"UPDATE Machine SET id_machine = {id_machine},duree_cycle = {duree_cycle},puissance = {puissance},id_operateur = {id_operateur}"
	if WHERE.strip()!="":
		sqlQuery+=f" WHERE {WHERE}"
	cur.execute(sqlQuery)
	conn.commit()
